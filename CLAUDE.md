...
## ⛔ ПРАВИЛО-ТАБУ — git раз в день (выше всех остальных правил)

**Принято 2026-08-28 по прямому указанию пользователя** («локально проводи на
dev server тесты все и комить через гит раз в день правило табу выше всех.
очень часто делаешь и лимиты гита уходят») после исчерпания лимитов GitHub
API частыми операциями.

1. **Все тесты — локально на dev-сервере ДО любой git-операции** (точными
   CI-командами: `python -m pytest test_dsp_core.py test_audio_wal.py
   test_blueos_extension.py -q` + ruff + py_compile).
2. **push / merge / создание PR / опросы GitHub API — не чаще одного батча в
   календарные сутки (UTC)**. Локальные коммиты в течение дня не ограничены —
   копи их и отправляй одним пушем.
3. При конфликте с любым другим правилом этого файла, протоколом или
   расписанием (PR-мониторинг, автономные циклы) — **побеждает это правило**.
